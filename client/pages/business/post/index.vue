<template>
  <v-layout row wrap>
    <v-toolbar flat prominent>
      ...
      <v-toolbar-title class="title-2">My Posts</v-toolbar-title>
      <v-spacer></v-spacer>
          ...
          <v-card>
            ...
            <v-list>
              <v-list-tile>
                ...
                <v-list-tile-title>
                  <nuxt-link to="DFF">Product</nuxt-link>
                </v-list-tile-title>
              </v-list-tile>
              ...
              <v-list-tile>
                ...
                <v-list-tile-title>
                  <nuxt-link to="/business/service/create">Service</nuxt-link>
                </v-list-tile-title>
              </v-list-tile>
              ...
              <v-list-tile>
                ...
                <v-list-tile-title>
                  <nuxt-link to="DFF">Offer</nuxt-link>
                </v-list-tile-title>
              </v-list-tile>
            </v-list>
          </v-card>
      ...
      <v-tabs height="60" v-model="active" color="grey lighten-3" slider-color="primary">
        ...
        <v-tab-item class="pl-0 pr-0 bg-light">
          <v-container grid-list-md class="pa-0">
            ...
            <v-layout row wrap>
              <v-flex xs12 sm6 md6 v-for="post in posts" :key="post">
                <v-card>
                  <v-img :src="post.image" :alt="post.title" content></v-img>
                      ...
                      <v-list-tile>
                        <v-list-tile-content>
                          <v-list-tile-title>
                            <nuxt-link
                              to="#"
                              class="black--text body-1 font-weight-medium"
                            >{{ post.title }}</nuxt-link>
                          </v-list-tile-title>
                          <v-list-tile-sub-title>1,000 views in this month</v-list-tile-sub-title>
                        </v-list-tile-content>
                      </v-list-tile>
                    </template>
                  </v-list>
                  ...
                  <v-card-actions>
                    <v-btn
                      ...
                      nuxt
                      :to="{path: '/business/post/promotion/'+post.slug}"
                    >
                    ...
                    <v-btn
                      v-if="post.type == 'producttype'"
                      icon
                      nuxt
                      :to="{path: '/business/post/product/'+post.id+'/edit/'+post.source_id}"
                    >
                      <v-icon>edit</v-icon>
                    </v-btn>

                    <v-btn
                      v-if="post.type == 'servicetype'"
                      icon
                      nuxt
                      :to="{path: '/business/post/service/'+post.id+'/edit/'+post.source_id}"
                    >
                      <v-icon>edit</v-icon>
                    </v-btn>
          ...
          <v-container grid-list-md class="pa-0">
            ...
            <v-layout row wrap>
             ...
             <v-flex xs12 sm6 v-for="product in products" :key="product">
                <v-card>
                  <v-img :src="product.image" :alt="product.title" content></v-img>
                  ...
                  <v-card-actions>
                    <v-btn
                      ...
                      class="text-capitalize"
                      nuxt
                      :to="{path: '/business/post/promotion/'+product.slug}"
                    >
                      <v-icon class="pr-2" color="primary">outlined_flag</v-icon>Promote
                    </v-btn>
                    <v-btn icon>
                    ...
                    <v-spacer></v-spacer>
                    <v-btn v-if="product.type == 'business'" icon nuxt to="/business/edit">
                      <v-icon>edit</v-icon>
                    </v-btn>
                    <v-btn
                      v-if="product.type == 'producttype'"
                      icon
                      nuxt
                      :to="{path: '/business/post/product/'+product.id+'/edit/'+product.source_id}"
                    >
                      <v-icon>edit</v-icon>
                    </v-btn>

                    <v-btn
                      v-if="product.type == 'servicetype'"
                      icon
                      nuxt
                      :to="{path: '/business/post/service/'+product.id+'/edit/'+product.source_id}"
                    >
                      <v-icon>edit</v-icon>
                    </v-btn>

                    <v-btn v-if="product.type != 'business'" icon @click="delete_post(product.id)">
                      <v-icon>delete</v-icon>
                    </v-btn>
                  </v-card-actions>
              ...
              <v-flex xs12 sm6 v-for="service in services" :key="service">
                <v-card flat class="bg-white">
                  ...
                  <v-card-actions>
                    <v-btn
                      flat
                      class="bg-light text-capitalize"
                      nuxt
                      :to="{path: '/business/post/promotion/'+service.slug}"
                    >
                      <v-icon class="pr-2" color="primary">outlined_flag</v-icon>Promote
                    </v-btn>
                    <v-btn icon>
                    ...
                    <v-btn
                      ico
                      nuxt
                      :to="{path: '/business/post/service/'+service.id+'/edit/'+service.source_id}"
                    >
                      <v-icon>edit</v-icon>
                    </v-btn>
        ...
        <v-tab-item class="pl-0 pr-0 bg-light"></v-tab-item>
              ...
              <v-flex xs12 sm6 v-for="promotion in promotions" :key="promotion">
                <v-card flat class="bg-white">
                  ...
                  <v-card-title>
                    ...
                    <span
                      class="grey--text"
                    >CPC : {{ promotion.cpc }}, Max Clicks : {{ promotion.max_click }}, Click : {{ promotion.clicked }}</span>
                  </v-card-title>

                  <v-card-actions>
                    <v-btn flat class="bg-light text-capitalize">
                      <v-icon class="pr-2" color="primary">calendar_today</v-icon>
                      Expire On :
                      {{ promotion.expire }}
                    </v-btn>
  ...
  methods: {
    async delete_product(e) {
      swal
        ...
        .then(result => {
          if (result.value) {
            this.$axios.post(`delete-post/` + e).then(response => {
              this.pro_alert = true
              this.type = 'info'
    ...
    async delete_promotion(e) {
      swal
        ...
        .then(result => {
          if (result.value) {
            this.$axios.post(`delete-promotion/` + e).then(response => {
